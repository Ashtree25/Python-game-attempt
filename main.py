...
# scoreboard
    ...
    def play(self):
        ...
        if self.is_collision(self.snake.x[0], self.snake.y[0], self.apple.x, self.apple.y):
            ...
            logging.debug(
                "After increase length, snake new length[%s]", self.snake.length)
            logging.debug(
                "BEFORE apple move, apple[%s, %s]", self.apple.x, self.apple.y)
            self.apple.move()
            logging.debug(
        ...
        while running:
            for event in pygame.event.get():
                if event.type == KEYDOWN:
                    ...
                    running = False
            OUTSIDE = self.check_boundary(
                self.snake.x[0], self.snake.y[0], OUTSIDE)
